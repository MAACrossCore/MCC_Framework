...
| 2026-09-20 | 每日探索「扫荡弹窗」识别改为固定标题 `^扫荡$` + ROI 留余量 | ✅ 已推送 `31cc438` | `tools/test_daily_stamina_dialog.py` |
| 2026-09-21 | 主线刷取「队伍行」OCR 变体容错 + 识别失败降级 | ✅ 已推送 `380ace9` / `c818d73` | `tools/test_mainstory_team.py` |
| 2026-09-20 | UI（MCCAvalonia）：更新检查限流不再弹窗，稳定版查询压成单次请求 | ✅ 已推送 `ae52d7a` | `docs/用户反馈排查手册.md` §5.1 |
| 2026-09-20 | 用户支持文档：`docs/用户反馈排查手册.md`（项目地图 / 日志位置 / 已确诊案例库） | ✅ 已推送 `2209968` | **`docs/用户反馈排查手册.md`** |
| 2026-09-21 | 周本「碎星虚影」领奖流程重做：报酬框挪位、新增「领奖后」选项、左滑找回 | ⏳ 已实现，**待实机验证** | **`docs/交接-周本碎星虚影领奖流程.md`** |
| 2026-09-22 | 主线刷取结算页：芯片仓满「确定」弹窗改为**容错优先**（`[JumpBack]` 前置）、`max_hit` 3→10、补有界收尾兜底 | ✅ 已推送 `c9f8898` | `tools/test_mainstory_settlement.py` |
| 2026-09-22 | 周本：次数改为「计算次数 + 自选次数」（读顶部 `x/600` 进度定目标，读不到就保持自选兜底）；`周本.json` 进度正则放宽到 `\d+/600` | ✅ 已实现，**待实机验证** | `tools/test_weekly_progress.py` |
| 2026-09-22 | 周本「领奖后」选项改名：`领取完奖励就结束（若未满则继续刷取直到领完）`——领完仍没满则按计算次数补满，满进度直接收工 | ✅ 已实现，**待实机验证** | `tools/test_weekly_reward.py` / `tools/test_weekly_progress.py` |
| 2026-09-22 | 周本左滑找回补有界兜底：`周本_找不到碎星虚影_收尾`（滑不动 → 干净回主页，不再 `NextList.Failed` 拖垮整队） | ✅ 已实现，**待实机验证** | `docs/周本流程分支表.md` |
